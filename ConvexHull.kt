...
class ConcaveHull {
    class Point(val x: Double, val y: Double) {
        ...
        override fun hashCode(): Int {
            ...
            val tmp = (y + (x + 1) / 2).toInt()
            return abs((x + tmp * tmp).toInt())
        }
    }
    ...
    private fun kNearestNeighbors(
        ...
        centerPoint: Point,
        k: Int
    ): ArrayList<Pair<Double, Point>> {
        val nearestList = ArrayList<Pair<Double, Point>>()
        for (point in points) {
            nearestList.add(Pair(euclideanDistance(centerPoint, point), point))
        }
    ...
    private fun findMinYPoint(points: ArrayList<Point>): Point {
    ...
    private fun sortByAngle(
        nearestPoints: ArrayList<Pair<Double, Point>>,
        currentPoint: Point,
        prevAngle: Double
    ): List<Point> {
        // Sort by angle descending
        val sortedList = nearestPoints.toMutableList().asReversed()
        sortedList.sortWith { o1, o2 ->
            ...
            if (compare == 0) {
                val firstDistance = o1.first ?: 0.0
                val secondDistance = o2.first ?: 0.0
                firstDistance.compareTo(secondDistance) ?: 0
            } else {
                compare
            }
        }
        return sortedList.map { it.second ?: Point(0.0, 0.0) }
    }
    ...
    fun calculateConcaveHull(
        ...
        val set = HashSet(pointArrayList)
        val pointArraySet = ArrayList(set)
        ...
        while ((currentPoint !== firstPoint || step == 2) && pointArraySet.size > 0) {
            ...
            val clockwisePoints =
                sortByAngle(kNearestPoints, currentPoint, previousAngle)
